...
function initBlogFiltering() {
  const buttons = Array.from(document.querySelectorAll('.blog-tag[data-blog-tag]'));
...
function initBlogShowMore() {
  ...
  const labelEl = document.querySelector('.blog-show-more-label');
  const chevron = byId('blog-chevron');
  const collapsedCards = Array.from(document.querySelectorAll('.blog-card--collapsed'));
  if (!button || !countEl || !labelEl || !chevron)
    return;
...
function initGitHubContributions() {
  ...
  const renderGitHubGrid = (sorted) => {
    if (!gridEl || !monthsEl || sorted.length === 0)
      return;
    const weeks = [];
    let currentWeek = [];
...
function initVisitorCounter() {
  ...
  const fetchCount = async (isFirst) => {
    try {
      ...
      if (!response.ok)
        throw new Error(`counterapi ${response.status}`);
      const data = await response.json();
      if (firstVisit) {
