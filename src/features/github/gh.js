...
export function initGitHubContributions() {
  ...
  const computeStreaks = (days = []) => {
    // days is a flat array of {date, count}
    let total = 0;
    let longest = 0;
    let current = 0;
  ...
  const renderGitHubGrid = (sorted) => {
    ...
    weeks.forEach((week, widx) => {
      ...
      week.forEach((day, dayIdx) => {
        if (!day) {
          ...
          const tooltipText = `${day.count} contribution${day.count !== 1 ? 's' : ''} on ${new Date(day.date).toLocaleDateString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}`;
          gridHtml += `<div class="gh-cell gh-level-${day.level}" data-contributions="${day.count}" data-date="${day.date}" title="${tooltipText}" style="animation-delay: ${(widx * 7 + dayIdx) * 20}ms"></div>`;
        }
      });
      gridHtml += '</div>';
    });
    ...
    // Add click handlers and enhanced tooltip for cells
    document.querySelectorAll('.gh-cell:not(.gh-empty)').forEach((cell) => {
      cell.addEventListener('click', () => {
        const contributions = cell.getAttribute('data-contributions');
        const date = cell.getAttribute('data-date');
        console.debug(`[GH] Clicked: ${contributions} contributions on ${date}`);
        cell.classList.add('gh-cell--clicked');
        setTimeout(() => cell.classList.remove('gh-cell--clicked'), 600);
      });
    });
  ...
  const updateStats = (total, weeks = []) => {
    ...
    // Calculate streaks
    const { current: currentStreak, longest: longestStreak } = computeStreaks(weeks);
    ...
    if (longestStreakEl) {
      animateValue(longestStreakEl, longestStreak, '');
    }
